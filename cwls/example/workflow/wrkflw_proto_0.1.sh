...
Example
  $scriptname -e youremail@ebi.ac.uk -s uniprot:wap_rat
  $scriptname -e youremail@ebi.ac.uk -s myseq.txt
  $scriptname -h
...
echo "Getting sequences for top 5 best hits from blast+ job..."
echo "Using dbfetch web service to get the sequences..."
echo "Running: cat $$.ids.txt | head -20 | sed 's/SP\://g;'| tr '\\n' ',' | ../dbfetch/dbfetch_lwp.pl fetchBatch uniprot - fasta raw >> $$.fasta"
