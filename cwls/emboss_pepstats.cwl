...
inputs:
  ...
  command:
    ...
    default:
      class: File
      location: ../../webservice-clients/python/emboss_pepstats.py
  ...
  termini:
    type: string?
    label: Include Termini Charges
    doc: "Include charges from the N-terminus and C-terminus when calculating the Isoelectric Point"
    inputBinding:
      prefix: --termini
      position: 9
    default: "true"
  ...
  mono:
    type: string?
    label: Monoisotopic Weights
    doc: "Use weight from the most abundant (prinicpal) isotope of each amino acid when calculating molecular weights. By default this is not enabled, so the average isotope weight is used instead."
    inputBinding:
      prefix: --mono
      position: 10
    default: "false"
