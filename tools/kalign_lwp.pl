...
=item *
L<http://www.ebi.ac.uk/Tools/webservices/services/msa/kalign_rest>
...
$Id: kalign_lwp.pl 2791 2014-05-29 15:31:39Z hpm $
...
GetOptions(
    ...
    'stype|m=s'    => \$tool_params{'stype'},   # Sequence type: DNA or protein
	'format|f=s'   => \$tool_params{'format'},  # Output format
	'gapopen|g=f'  => \$tool_params{'gapopen'}, # Gap creation penalty
	'gapext|e=f'   => \$tool_params{'gapext'},  # Gap extension penalty
	'termgap|t=f'  => \$tool_params{'termgap'}, # Terminal gap penalty
	'bonus|b=f'    => \$tool_params{'bonus'},   # Bonus score
    'sequence=s'   => \$params{'sequence'},     # Input sequences/alignment
    
    # Compatability options (old command-line)
    'moltype=s'   => \$params{'moltype'},    # Molecule type
    'gpo=f'       => \$params{'gpo'},        # Gap open penalty
    'gpe=f'       => \$params{'gpe'},        # Gap extension penalty
    'tgpe=f'      => \$params{'tgpe'},       # Terminal gap penalty
...
sub load_params {
	...
	# Handle compatability options
	if(!$tool_params{'stype'} && $params{'moltype'}) {
		if($params{'moltype'} =~ m/^P/i) {
			$tool_params{'stype'} = 'protein';
		}
		elsif($params{'moltype'} =~ m/^[DR]NA/i) {
			$tool_params{'stype'} = 'dna';
		}
		else {
			die 'Error: unknown value "' . $params{'moltype'} . '" for moltype.';
		}
	}
	if(!$tool_params{'gapopen'} && $params{'gpo'}) {
		$tool_params{'gapopen'} = $params{'gpo'};
	}
	if(!$tool_params{'gapext'} && $params{'gpe'}) {
		$tool_params{'gapext'} = $params{'gpe'};
	}
	if(!$tool_params{'termgap'} && $params{'tgpe'}) {
		$tool_params{'termgap'} = $params{'tgpe'};
	}
